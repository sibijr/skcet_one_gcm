<?php
 class user{
	...
	/*public function update_placement($dept,$fields = array()){
 		if(!$this->_db->update_table('placement',$dept,$fields)){
				throw new Exception("there was a problem while updating admission.");
 		}
 	}*/
	...
	public function adviser_tutor($fields = array()){
 		if(!$this->_db->insert('adviser_tutor',$fields)){
 			throw new Exception("There was a problem in creating an adviser/tutor account!");
 		}
 	}
	...
	public function find_reg($reg = null){
 		if($reg){
 			$field = '`reg_no`';
 			$data = $this->_db->get('users',array($field, '=' ,$reg ));
 			if($data->count()){
 				$this->_data = $data->first();
 				return true;
 			}
	...
	public function find_response($user = null){
 	...
 	public function login($username = null,$password = null,$remember = false){
	 	...
	 	public function login_response($reg_no = null,$password = null){
	 		$user = $this->find_reg($reg_no);
	 		if($user){
	 			if($this->data()->password === hash::make($password,$this->data()->salt)){
	 				return $this->data()->id;
	 			}	
